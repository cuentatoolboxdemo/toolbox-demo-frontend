...
export function AnimatedAvatar({ isShrunk, isThinking }: AnimatedAvatarProps) {
    return (
        <div
            className={`absolute inset-0 flex items-center justify-center pointer-events-none transition-all duration-1000 ease-[cubic-bezier(0.4,0,0.2,1)] z-0 ${isShrunk ? "scale-[1.8] opacity-[0.03] md:scale-[1.5]" : "scale-100 opacity-100"
                }`}
        >
            <div className="relative flex items-center justify-center">
                {/* Core Glow */}
                <div
                    className={`rounded-full bg-blue-500 blur-[40px] transition-all duration-700 ${isThinking ? "w-48 h-48 animate-pulse bg-indigo-500" : "w-32 h-32 opacity-70"
                        }`}
                />
